...
const { advertRequestAdmin, advertRequestConfirmation } = require('../../services/growtiva/emailTemplates');
...
router.post('/', async (req, res) => {
  const {
    ...
    ad_type, budget, want_hard_copy, additional_details,
  } = req.body;

  if (!full_name || !email || !business_name || !ad_type || !budget) {
    return res.status(400).json({
      error: 'full_name, email, business_name, ad_type, and budget are required.',
    });
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  ...
  try {
    ...
    await sendAdminNotification(`📣 New Advert Request from ${full_name}`, advertRequestAdmin(data), GROWTIVA_EMAIL_TO);
    await sendConfirmation(email, 'We received your advert request — Growtiva', advertRequestConfirmation(data));

    res.status(201).json({ success: true, message: 'Advert request submitted successfully.' });
  } catch (err) {
...
router.get('/', adminAuth, async (req, res) => {
  try {
    ...
    const [records, total] = await Promise.all([
      AdvertRequest.find(filter).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)),
