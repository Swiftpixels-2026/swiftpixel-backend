...
const { profileAdmin, profileConfirmation } = require('../../services/growtiva/emailTemplates');
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    await sendAdminNotification(`👤 New Profile from ${full_name}`, profileAdmin(data), GROWTIVA_EMAIL_TO, 'growtiva');
    await sendConfirmation(email, 'Profile received — Growtiva', profileConfirmation(data), 'growtiva');

    res.status(201).json({ success: true, message: 'Profile submitted successfully.' });
  } catch (err) {
