...
const { contactAdmin, contactConfirmation } = require('../../services/growtiva/emailTemplates');
...
router.post('/', async (req, res) => {
  ...
  try {
    const record = await ContactMessage.create({ name, email, subject, message });
    const data = record.toObject();

    await sendAdminNotification(`✉️ New Growtiva Contact from ${name}`, contactAdmin(data), GROWTIVA_EMAIL_TO, 'growtiva');
    await sendConfirmation(email, "We got your message — Growtiva", contactConfirmation(data), 'growtiva');
